...
parser = argparse.ArgumentParser(description='This is the command-line interface for NINJA-SHI7', usage='python ninja_shi7.py -i <input> -o <output> -t <threads>...')
...
parser.add_argument('-o', '--output', help='Set the directory path of the destination (default: the directory path where your input is located)')
...
if args.adaptor_type == 'None':
	...
	for f in R1_fq:
		subprocess.call('cd ' + fastq_path + '/temp && java -jar ' + script_path + '\Trimmomatic-0.36\\trimmomatic-0.36.jar PE ' + f + ' ' + re.sub('R1','R2',f) + ' ' + re.sub('R1','fwdp',f) + ' \dev\\null ' + re.sub('R1','revp',f) + ' \dev\\null ILLUMINACLIP:' + script_path + '\Trimmomatic-0.36\\adapters\NexteraPE-PE.fa:2:30:10:2:true -threads ' + args.threads, shell=True)
else:
	for f in R1_fq:
		print('cd ' + fastq_path + '/temp && java -jar ' + script_path + '\Trimmomatic-0.36\\trimmomatic-0.36.jar PE ' + f + ' ' + re.sub('R1','R2',f) + ' ' + re.sub('R1','fwdp',f) + ' \dev\\null ' + re.sub('R1','revp',f) + ' \dev\\null ILLUMINACLIP:' + script_path + '\Trimmomatic-0.36\\adapters\TruSeq3-PE-2.fa:2:30:10:2:true -threads ' + args.threads)
		subprocess.call('cd ' + fastq_path + '/temp && java -jar ' + script_path + '\Trimmomatic-0.36\\trimmomatic-0.36.jar PE ' + f + ' ' + re.sub('R1','R2',f) + ' ' + re.sub('R1','fwdp',f) + ' \dev\\null ' + re.sub('R1','revp',f) + ' \dev\\null ILLUMINACLIP:' + script_path + '\Trimmomatic-0.36\\adapters\TruSeq3-PE-2.fa:2:30:10:2:true -threads ' + args.threads, shell=True)
...
if args.flash == 'enabled':
	for f in FWDP:
		s = 'cd ' + fastq_path + '/temp && ' + script_path + '/flash.exe ' + f + ' ' + re.sub("fwdp","revp",f) + ' -o ' + re.sub('_fwdp.+.fastq','',f) + ' -M ' + args.max_overlap + ' -m ' + args.min_overlap
...
if args.trimmer == 'enabled':
	...
	for f in TRIMMER:
		...
		s2 = script_path + '/ninja_shi7.exe ' + f + ' ' + re.sub('.extendedFrags.fastq','.trimmed.fastq',f) + ' ' + args.trim_length + ' ' + args.trim_qual + ' ' + args.trim_qual + 'FLOOR 5 ASS_QUALITY 30 >> ninja_shi7_report.log'
...
if args.output != None:
	...
	fna_path = os.path.dirname(fastq_path) + '/seqs.fna'
