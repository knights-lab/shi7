...
def main():
	...
	if args.flash == 'enabled':
		for f in FWDP:
			s = 'cd ' + os.path.join(os.path.dirname(fna_path), 'temp') + '; ' + os.path.join(script_path,'FLASH-1.2.11','flash ') + f + ' ' + re.sub("fwdp","revp",f) + ' -o ' + re.sub('_fwdp.+.fastq','',f) + ' -M ' + args.max_overlap + ' -m ' + args.min_overlap
	...
	if args.trimmer == 'enabled':
		...
		for f in TRIMMER:
			...
			s2 = os.path.join(script_path,'ninja_shi7_mac ') + f + ' ' + re.sub('.extendedFrags.fastq','.trimmed.fastq',f) + ' ' + args.trim_length + ' ' + args.trim_qual + ' ' + args.trim_qual + 'FLOOR 5 ASS_QUALITY 30 >> ninja_shi7_report.log'
